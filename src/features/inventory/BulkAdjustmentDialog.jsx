  ...
  const handleChange = async (index, field, value) => {
    ...
    if (
      (field === "quantityChange" ||
        field === "productId" ||
        field === "locationId") &&
      Number(current.quantityChange) < 0 &&
      current.locationId &&
      products?.find((p) => p.productId === current.productId)
        ?.serialNoApplicable
    ) {
      try {
        showLoader();
        ...
        hideLoader();
        serials?.filter((x) => x.location_id === current.locationId)?.length ===
          0 && showSnackbar("No instock serial found!", "error");
        updated[index].serialOptions = serials
          ?.filter((x) => x.location_id === current.locationId)
          ?.map((s) => {
            return {
      ...
      if (product?.serialNoApplicable) {
        if (quantity > 0) {
          ...
          if (serialArray.length !== quantity) {
            ...
            errorMsg = `Row ${
              i + 1
            }: Serial numbers count must match quantity (${quantity}).`;
            break;
          }
        }
        ...
        if (quantity < 0) {
          if ((a.serialNumbers?.length || 0) !== Math.abs(quantity)) {
            hasError = true;
            errorMsg = `Row ${i + 1}: You must select exactly ${Math.abs(
              quantity
            )} serial number(s).`;
            break;
          }
      ...
      <DialogContent dividers>
        {adjustments.map((adj, index) => (
            ...
            <Grid container spacing={2} alignItems="center">
              ...
              <Grid item xs={12} sm={6} md={3}>
                <Autocomplete
                  ...
                  options={[
                    ...new Map(
                      products.map((p) => [p.productCategoryId, p])
                    ).values(),
                  ]}
                    ...
                    adj.selectedCategory
                      ? {
                          ...
                          productCategoryName:
                            products.find(
                              (p) =>
                                p.productCategoryId === adj.selectedCategory
                            )?.productCategoryName || "-",
                        }
                      : null
                  }
              ...
              <Grid item xs={12} sm={6} md={3}>
                <Autocomplete
                  ...
                  value={
                    products.find((p) => p.productId === adj.productId) || null
                  }
                  onChange={(e, newValue) =>
                    handleChange(
                      index,
                      "productId",
                      newValue ? newValue.productId : ""
                    )
                  }
                  renderInput={(params) => (
              ...
              <Grid item xs={12} sm={6} md={3}>
                <Autocomplete
                  ...
                  value={
                    locations.find((l) => l.locationId === adj.locationId) ||
                    null
                  }
                  onChange={(e, newValue) =>
                    handleChange(
              ...
              <Grid item xs={6} sm={3} md={2}>
                <TextField
                  ...
                  onChange={(e) =>
                    handleChange(index, "quantityChange", e.target.value)
                  }
                  InputProps={{
                    endAdornment: products?.find(
                      (p) => p.productId === adj.productId
                    )?.unit ? (
                      <InputAdornment position="end">
                        {
                          products?.find((p) => p.productId === adj.productId)
                            ?.unit
                        }
                      </InputAdornment>
                    ) : null,
          ...
          <Typography>{products?.find((p) => p.productId === adj.productId)?.serialNoApplicable ? 'Yes' : 'No'}</Typography>
              ...
              {products.find((p) => p.productId === adj.productId)
                ?.serialNoApplicable === true &&
                Number(adj.quantityChange) < 0 && (
                  <Grid item xs={12}>
                    <Autocomplete
                      multiple
                      size="small"
                      ...
                      onChange={(e, newValue) =>
                        handleChange(index, "serialNumbers", newValue)
                      }
                      renderInput={(params) => (
                        <TextField
                          {...params}
                          ...
                          error={
                            !!adj.quantityChange &&
                            adj.serialNumbers?.length !==
                              Math.abs(Number(adj.quantityChange))
                          }
                          helperText={
                            !!adj.quantityChange &&
                            (adj.serialNumbers?.length !==
                            Math.abs(Number(adj.quantityChange))
                              ? `You must select exactly ${Math.abs(
                                  Number(adj.quantityChange)
                                )} serial number(s)`
                              : "")
                          }
                        />
                ...
                Number(adj.quantityChange) > 0 && (
                  <Grid item xs={12} sm={12} md={12}>
                    <TextField
                      ...
                      onChange={(e) =>
                        handleChange(index, "serialNumbersAdd", e.target.value)
                      }
                      error={
                        !!adj.quantityChange &&
                        adj.serialNumbersAdd?.split(",").filter((s) => s.trim())
                          .length !== Number(adj.quantityChange)
                      }
                      helperText={
                        !!adj.quantityChange &&
                        (adj.serialNumbersAdd
                          ?.split(",")
