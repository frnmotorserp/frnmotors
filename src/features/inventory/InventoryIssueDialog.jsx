...
export default function InventoryIssueDialog({
  ...
  const handleItemChange = async (index, field, value) => {
    ...
    if (field === "product" || field === "quantity") {
      ...
      if (
        ...
        location &&
        current?.product?.serial_no_applicable === true &&
        !current?.serialOptions?.length
      ) {
        try {
          showLoader();
          ...
          hideLoader();
          serials?.filter((x) => x.location_id === location.locationId)
            ?.length === 0 && showSnackbar("No instock serial found!", "error");
          updated[index].serialOptions = serials
            ?.filter((x) => x.location_id === location.locationId)
            ?.map((s) => {
              return {
  ...
  const handleSubmit = async () => {
    if (
      ...
      showSnackbar(
        "Please fill required fields - Issue Date, Location, Issued To, Remarks!",
        "warning"
      );
      return;
    }
    ...
    const invalidProductQantity = items.filter(
      (it) => Number(it.quantity || 0) <= 0
    );
    if (invalidProductQantity?.length > 0) {
      showSnackbar(
        "You should input positive quantity for all the items!",
        "warning"
      );
      return;
    }

    const itemHaveSerialNumbersMismatch = items.filter(
      (it) =>
        it?.product?.serial_no_applicable &&
        it?.serialNumbers?.length != Number(it?.quantity || 0)
    );

    if (itemHaveSerialNumbersMismatch?.length > 0) {
      showSnackbar(
        "For Items has serial number the count of selected serial numbers should be exactly same as item quantity!",
        "warning"
      );
      return;
    }
    ...
    <Dialog open={open} onClose={handleDialogClose} maxWidth="md" fullWidth>
      ...
      <DialogContent dividers>
        ...
        <Alert severity="warning" sx={{ mb: 2 }}>
          <strong>Note:</strong> Do not enter negative numbers. Issuing 1 unit
          means 1 unit will be deducted from stock. Negative values (e.g., -1)
          are invalid and not allowed.
        </Alert>
                ...
                <Grid container spacing={2}>
                  <Grid item xs={6}>
                    <Autocomplete
                      ...
                      value={item.category}
                      onChange={(e, val) =>
                        handleItemChange(idx, "category", val)
                      }
                      renderInput={(params) => (
                  ...
                  <Grid item xs={6}>
                    <Autocomplete
                      ...
                      options={products.filter(
                        (p) =>
                          p.location_id === location.locationId &&
                          (!item.category ||
                  ...
                  {item?.product?.serial_no_applicable && (
                    <Grid item xs={10}>
                      <Autocomplete
                        ...
                        onChange={(e, newValue) =>
                          handleItemChange(idx, "serialNumbers", newValue)
                        }
                        renderInput={(params) => (
                          <TextField
                            {...params}
        ...
        {location && (
          <Button sx={{ mt: 2 }} variant="outlined" onClick={handleAddItem}>
            Add Another Item
          </Button>
        )}
      </DialogContent>
