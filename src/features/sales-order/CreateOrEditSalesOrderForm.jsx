...
import {
  Dialog, DialogTitle, DialogContent, DialogActions, ToggleButtonGroup, ToggleButton, Autocomplete,
  Button, TextField, Box, Grid, Typography, MenuItem, FormControlLabel, FormLabel, RadioGroup, Radio,
...
const CreateOrEditSalesOrderForm = ({ open, handleClose, mode, editData, onSuccess, companyList, customerList, stateList, dealerList, locationList, userList }) => {
  ...
  const fetchAvailableSaleableItems = async (locationId, hideSnackbar = false) => {
  ...
  return (
    <Dialog open={open} onClose={handleClose} fullWidth maxWidth="lg">
      <DialogTitle>{mode === 'edit' ? 'Edit Sales Order' : 'Create Sales Order'}</DialogTitle>
      <DialogContent dividers style={{ maxHeight: '80vh' }}>
        <Grid container spacing={2}>
          ...
          <Grid item xs={12}>
            <Typography variant="subtitle2" sx={{ mb: 1 }}>Select Company</Typography>
            <ToggleButtonGroup
                ...
                >
                  <Box display="flex" flexDirection="column" width="100%">
                    <Box display="flex" alignItems="center" mb={1}>
                      <BusinessIcon sx={{ mr: 1, color: orderData.companyId === company.companyId ? '#fff' : '#1976d2' }} />
          ...
          <Grid item xs={12} md={12}>
            <FormLabel component="legend" sx={{ fontSize: 12, mb: 1 }}>Buyer Type</FormLabel>
            <ToggleButtonGroup
              color="primary"
          ...
          {orderData.orderType === "CUSTOMER" && (
            <Grid item xs={12}>
              <Typography variant="subtitle2" sx={{ mb: 1 }}>Select Customer</Typography>
              <Grid container spacing={1}>
                <Grid item xs={9} md={10}>
                  <Autocomplete
                    options={customerList}
                    getOptionLabel={(option) => option.customerName || ""}
                    value={customerList.find(c => c.customerId === orderData.customerId) || null}
                    onChange={(e, newValue) => {
                      handleFieldChange('customerId', newValue ? newValue.customerId : '');
                      setSelectedCustomer(customerList.find(c => c.customerId === newValue.customerId) || null)
                    }}
            ...
            <Grid item xs={12} sx={{ mt: 1 }}>
              ...
              >
                ...
                >
                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <PersonIcon sx={{ fontSize: 18, color: 'primary.main', mr: 1 }} />
                    <Typography variant="body2"><strong>Name:</strong> {selectedCustomer?.customerName}</Typography>
                  </Box>

                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <BadgeIcon sx={{ fontSize: 18, color: 'text.secondary', mr: 1 }} />
                    <Typography variant="body2"><strong>Code:</strong> {selectedCustomer?.customerCode}</Typography>
                  </Box>

                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <PhoneIcon sx={{ fontSize: 18, color: 'success.main', mr: 1 }} />
                    <Typography variant="body2"><strong>Phone:</strong> {selectedCustomer?.phone}</Typography>
                  </Box>

                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <EmailIcon sx={{ fontSize: 18, color: 'info.main', mr: 1 }} />
                    <Typography variant="body2"><strong>Email:</strong> {selectedCustomer?.email}</Typography>
                  </Box>

                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <GavelIcon sx={{ fontSize: 18, color: 'warning.main', mr: 1 }} />
                    <Typography variant="body2"><strong>GSTIN:</strong> {selectedCustomer?.gstin}</Typography>
                  </Box>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    <Typography variant="body2">
                      <strong>Status:</strong> {selectedCustomer?.status === "true" ? "Active" : "Inactive"}
                    </Typography>
                  </Box>
                </Box>
              </Box>
              ...
              >
                ...
                <TextField
                  ...
                  onChange={(e) => handleFieldChange('billingStateCode', e.target.value)}
                  fullWidth
                  size="small"
                  InputProps={{
                    startAdornment: <PlaceIcon fontSize="small" sx={{ mr: 1, color: 'text.secondary' }} />,
                  }}
                >
              ...
              >
                ...
                <TextField
                  ...
                  onChange={(e) => handleFieldChange('shippingStateCode', e.target.value)}
                  fullWidth
                  size="small"
                  InputProps={{
                    startAdornment: <PlaceIcon fontSize="small" sx={{ mr: 1, color: 'text.secondary' }} />,
          ...
          {orderData.orderType === "DEALER" && (
            <Grid item xs={12}>
              <Typography variant="subtitle2" sx={{ mb: 1 }}>Select Dealer</Typography>
              <Grid container spacing={1}>
                <Grid item xs={9} md={10}>
                  <Autocomplete
                    options={dealerList}
                    getOptionLabel={(option) => option.dealerName || ""}
                    value={dealerList.find(d => d.dealerId === orderData.dealerId) || null}
                    onChange={(e, newValue) => {
                      handleFieldChange('dealerId', newValue ? newValue.dealerId : '');
                      setSelectedDealer(dealerList.find(d => d.dealerId === newValue?.dealerId) || null);
                    }}
                    renderOption={(props, option) => (
            ...
            <Grid item xs={12} sx={{ mt: 1 }}>
              ...
              >
                ...
                >
                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <PersonIcon sx={{ fontSize: 18, color: 'primary.main', mr: 1 }} />
                    <Typography variant="body2"><strong>Name:</strong> {selectedDealer?.dealerName}</Typography>
                  </Box>

                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <BadgeIcon sx={{ fontSize: 18, color: 'text.secondary', mr: 1 }} />
                    <Typography variant="body2"><strong>Code:</strong> {selectedDealer?.dealerCode}</Typography>
                  </Box>

                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <PhoneIcon sx={{ fontSize: 18, color: 'success.main', mr: 1 }} />
                    <Typography variant="body2"><strong>Phone:</strong> {selectedDealer?.phone}</Typography>
                  </Box>

                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <EmailIcon sx={{ fontSize: 18, color: 'info.main', mr: 1 }} />
                    <Typography variant="body2"><strong>Email:</strong> {selectedDealer?.email}</Typography>
                  </Box>

                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <GavelIcon sx={{ fontSize: 18, color: 'warning.main', mr: 1 }} />
                    <Typography variant="body2"><strong>GSTIN:</strong> {selectedDealer?.gstin}</Typography>
                  </Box>

                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <CreditCardIcon sx={{ fontSize: 18, color: 'secondary.main', mr: 1 }} />
                    <Typography variant="body2"><strong>PAN:</strong> {selectedDealer?.pan}</Typography>
                  </Box>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    <Typography variant="body2">
                      <strong>Status:</strong> {selectedDealer?.status ? "Active" : "Inactive"}
                    </Typography>
                  </Box>
                </Box>
              </Box>
                ...
                >
                  ...
                  {selectedDealer.addresses.map((addr, index) => (
                    ...
                    >
                      ...
                      >
                        <Typography variant="body2"><strong>Type:</strong> {addr.addressType}</Typography>
                        <Typography variant="body2"><strong>Primary:</strong> {addr.isPrimary ? 'Yes' : 'No'}</Typography>
                        <Typography variant="body2"><strong>State:</strong> {addr.stateName}</Typography>

                        <Typography variant="body2"><strong>District:</strong> {addr.district}</Typography>
                        <Typography variant="body2"><strong>City:</strong> {addr.city}</Typography>
                        <Typography variant="body2"><strong>Country:</strong> {addr.country}</Typography>

                        <Typography variant="body2" sx={{ gridColumn: 'span 3' }}>
                          <strong>Address:</strong> {addr.addressLine1}, {addr.addressLine2}
                        </Typography>

                        <Typography variant="body2"><strong>Pincode:</strong> {addr.pincode}</Typography>
                      </Box>
          ...
          {(orderData?.customerId || orderData?.dealerId) && deliveryApplicable === 'Yes' &&
            <Grid item xs={12}>
          ...
          {(orderData?.customerId || orderData?.dealerId) && <Grid item xs={12}>
            <Box sx={{ mb: 2 }}>
              <FormControl component="fieldset">
                <FormLabel component="legend">Is Delivery Applicable?</FormLabel>
                <RadioGroup
                  row
          ...
          {(orderData?.customerId || orderData?.dealerId) && <Grid item xs={12} md={6}>
            <TextField
              select
              size='small'
              fullWidth
            ...
            >
              {locationList?.map((loc) => (
                <MenuItem key={loc.locationId} value={loc.locationId}>
                  {loc.locationName} — {loc.address}, {loc.districtName}, {loc.stateName} - {loc.pincode}
                </MenuItem>
              ))}
            </TextField>
          </Grid>}
          {(orderData?.customerId || orderData?.dealerId) && <Grid item xs={12} md={6}>
            <TextField
              ...
              onChange={(e) => handleFieldChange('bookedByUserId', e.target.value)}
            >
              {userList?.map((user) => (
                <MenuItem key={user.userId} value={user.userId}>
                  {[user.userFirstname, user.userMiddlename, user.userLastname].filter(Boolean).join(' ')}
                </MenuItem>
              ))}
            </TextField>
          </Grid>}
