  ...
  const getStateAndDistrictListAPICall = (hideSnackbar = false) => {
    ...
    Promise.all([getStateListService()])
      ...
      .catch((error) => {
        ...
        hideLoader();
        !hideSnackbar &&
          showSnackbar("Failed to fetch state or district list!", "error");
      });
  };
    ...
    <PageWrapper title={"Vendor Management"} actionButtons={ActionButtonsArr}>
      <Box p={3}>
        ...
        {vendors.map((vendor) => (
          <Grid item xs={12} sm={6} md={4} key={vendor.vendorId}>
            <Card>
              ...
              <CardActions>
                <Button startIcon={<EditIcon />} onClick={() => handleOpenDialog(vendor, false)}>Edit</Button>
                <Button startIcon={<VisibilityIcon />} onClick={() => handleOpenDialog(vendor, true)}>View</Button>
              </CardActions>
            </Card>
          </Grid>
        ))}
                ...
                >
                  <CardContent sx={{ flexGrow: 1 }}>
                    ...
                    >
                      ...
                      <Chip
                        size="small"
                        color={vendor.status === true ? "primary" : "error"}
                        sx={{ fontSize: "0.6rem" }}
                        label={vendor.status === true ? "Active" : "In-Active"}
                      ></Chip>
                    </Box>
              ...
              {activeTab === 0 && (
                <Stack spacing={2} mt={2}>
                  ...
                  <TextField
                    ...
                    error={!!errors.gstin}
                    helperText={
                      errors.gstin && "Either GSTIN or PAN is required"
                    }
                    size={"small"}
                  />
                  <TextField
        ...
        <InvoicePaymentSummaryDialog
          ...
          orders={invoiceAndPayments || []}
          vendorName={vendorBuyerName}
          vendorId={vendorIdStoreCurrent}
        />
      </Box>
    </PageWrapper>
  );
